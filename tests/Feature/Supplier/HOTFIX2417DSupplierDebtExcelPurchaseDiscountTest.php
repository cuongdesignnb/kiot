...
class HOTFIX2417DSupplierDebtExcelPurchaseDiscountTest extends TestCase
{
    ...
    public function test_purchase_item_discount_is_exported_to_discount_column(): void
    {
        ...
        $this->assertEquals($lineTotal, (int) ($itemRow['I'] ?? 0), 'Thành tiền must equal qty*price - discount');
    }
    ...
    public function test_purchase_discount_does_not_change_debt_amount(): void
    {
        ...
        $this->assertEquals(9_000_000, (int) ($docRow['J'] ?? 0), 'Ghi nợ on doc row must match net of purchases.discount');
        $this->assertEmpty($docRow['K'] ?? '', 'Ghi có on doc row must remain empty for a purchase');
        // Discount summary row must NOT carry Ghi nợ / Ghi có.
        $this->assertEmpty($discRow['J'] ?? '', 'discount summary row must NOT touch Ghi nợ');
        $this->assertEmpty($discRow['K'] ?? '', 'discount summary row must NOT touch Ghi có');
    }
