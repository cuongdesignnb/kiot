 ...
 *
 * The supplier list is intentionally lightweight. The selected supplier's
 * canonical balance is loaded through the authenticated debt-display
 * endpoint, while this suite also confirms the backend formula stayed
 * exactly the same (overpayment still flows through as a negative
 * debt_amount).
 */
class HOTFIX2421PurchaseSupplierBalanceDisplayTest extends TestCase
{
    ...
    // ── TC-01 — /purchases/create uses lightweight rows plus lazy debt ──
    public function test_purchase_create_suppliers_use_lightweight_rows_and_lazy_debt_endpoint(): void
    {
        $admin = $this->admin();
        ...
        $row = collect($props['suppliers'] ?? [])->firstWhere('id', $sup->id);
        $this->assertNotNull($row, 'supplier code reached the FE');
        $this->assertArrayNotHasKey('supplier_debt_amount', $row);
        $this->assertArrayNotHasKey('supplier_picker_display_balance', $row);

        $debt = $this->actingAs($admin)
            ->getJson("/purchases/suppliers/{$sup->id}/debt-display")
            ->assertOk()
            ->json();
        $this->assertSame(1_500_000.0, (float) $debt['debt_stored_projection']['supplier_payable']);
        $this->assertTrue($debt['debt_has_mismatch']);
        $this->assertEquals(
            PartnerDebtDisplayBalance::responseAliases($sup->fresh()),
            collect($debt)->except('id')->all()
        );
    }

    // ── TC-02 — /api/suppliers/search stays lightweight ──
    public function test_supplier_search_response_stays_lightweight(): void
    {
        $admin = $this->admin();
