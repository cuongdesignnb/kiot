...
[$script, $basePath, $offsetId, $actorId, $versionToken, $idempotencyKey, $barrierDirectory, $workerName] = $argv;
...
$app->instance(DebtOffsetFailureInjector::class, new class extends DebtOffsetFailureInjector
{
    public function hit(string $point): void
    {
        if ($point === 'AFTER_PARTNER_BALANCE_UPDATE') {
            usleep(400_000);
        }
    }
});
...
try {
    ...
    $result = $app->make(DebtOffsetWorkflowService::class)->apply(
        $offset,
        $actor,
        $versionToken,
        $idempotencyKey,
