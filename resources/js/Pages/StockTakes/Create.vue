...
const selectProduct = (product) => {
    ...
    if (!existing) {
        items.value.unshift({ 
            ...
            actual_stock: product.stock_quantity || 0 // Default actual stock to system stock
        });
    }
    searchQuery.value = '';
...
const totalActualQty = computed(() => itemsWithDiffs.value.reduce((sum, item) => sum + (parseInt(item.actual_stock) || 0), 0));
const totalDiffDecreaseValue = computed(() => itemsWithDiffs.value.filter(i => i.diff_qty < 0).reduce((sum, item) => sum + item.diff_value, 0));
const totalDiffValue = computed(() => itemsWithDiffs.value.reduce((sum, item) => sum + item.diff_value, 0));
...
const save = async (status) => {
    ...
    try {
        await router.post('/stock-takes', {
            ...
            status: status, // 'draft' | 'balanced'
            action_date: transactionDate.value,
            note: note.value,
            items: itemsWithDiffs.value
        });
    } catch (e) {
...
<template>
    ...
    <div class="h-screen flex flex-col bg-[#eef1f5] text-[13px] overflow-hidden font-sans">
        ...
        <header class="bg-[#005bb5] text-white px-4 h-[50px] flex items-center justify-between shadow-sm flex-shrink-0">
            <div class="flex items-center gap-4 flex-1">
                <Link href="/stock-takes" class="text-white hover:text-blue-100 transition-colors flex items-center gap-2 font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg> Kiểm kho
                </Link>
                ...
                <div class="relative w-full max-w-[600px] ml-4">
                    ...
                    <input v-model="searchQuery" @focus="showSuggestions = true" @blur="hideSuggestions" type="text" class="w-full pl-9 pr-12 py-[7px] border-none text-gray-800 rounded-sm focus:outline-none focus:ring-2 focus:ring-blue-300 bg-white" placeholder="Tìm hàng hóa theo mã hoặc tên (F3)">
        ...
        <div class="bg-gray-100 flex items-center px-4 py-1.5 text-gray-500 border-b border-gray-200">
            <span class="mr-6 cursor-pointer font-medium text-blue-600 border-b-2 border-blue-600 pb-1 -mb-1.5">Tất cả ({{ items.length }})</span>
            <span class="mr-6 cursor-pointer hover:text-gray-800 transition-colors">Khớp ({{ itemsWithDiffs.filter(i => i.diff_qty === 0).length }})</span>
            <span class="mr-6 cursor-pointer hover:text-gray-800 transition-colors">Lệch ({{ itemsWithDiffs.filter(i => i.diff_qty !== 0).length }})</span>
            <span class="cursor-pointer hover:text-gray-800 transition-colors">Chưa kiểm (0)</span>
        </div>
                ...
                <div class="flex-1 overflow-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody v-if="items.length > 0">
                            <tr v-for="(item, index) in itemsWithDiffs" :key="item.product_id" class="border-b border-gray-100 hover:bg-[#f0f9ff]/40 transition-colors">
                                ...
                                <td class="p-3 text-center w-28">
                                    <input type="number" v-model="item.actual_stock" min="0" class="w-20 border border-gray-300 rounded-sm py-1.5 px-2 text-right outline-none focus:border-blue-500 text-[13px] transition-colors mx-auto block font-semibold shadow-inner bg-blue-50/30">
                                </td>
                                <td class="p-3 text-right font-medium w-24" :class="{'text-red-500': item.diff_qty < 0, 'text-green-500': item.diff_qty > 0}">{{ item.diff_qty }}</td>
                                <td class="p-3 font-bold text-gray-800 text-right w-[120px]">{{ formatCurrency(item.diff_value) }}</td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="flex-1 overflow-auto bg-gray-50 flex flex-col">
                    ...
                    <div class="p-4 flex flex-col gap-4 bg-white border-b border-gray-200 flex-1">
                        ...
                        <div class="flex items-center gap-3 pt-2">
                            <label class="font-medium text-gray-700 w-[100px]">Tổng SL thực tế</label>
                    ...
                    <div class="p-4 flex-1">
                         <div class="bg-blue-50/50 text-blue-800 font-medium py-2 px-3 rounded-t text-center text-[13px] border-b border-blue-100 pb-2">
                             Kiểm gần đây
                         </div>
                         <div class="bg-white border-x border-b border-gray-200 h-64 rounded-b p-4 flex items-center justify-center text-gray-400">
                             <!-- Placeholder for recent checks -->
                         </div>
                    </div>
