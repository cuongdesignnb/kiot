...
const openCreateModal = async () => {
    ...
    try {
        const res = await axios.get("/api/my-tasks/categories");
...
const submitCreate = async () => {
    ...
    creating.value = true;
    try {
        await axios.post("/api/my-tasks", {
            title: createForm.value.title,
            description: createForm.value.description || null,
            category_id: createForm.value.category_id || null,
        ...
        <div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-xl mx-4">
                <div class="flex items-center justify-between px-6 py-4 border-b">
                    <h2 class="text-lg font-bold">Tạo công việc</h2>
                ...
                <div class="px-6 py-5 space-y-4">
                    ...
                    <div>
                        ...
                        <input v-model="createForm.title" type="text" maxlength="255" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none" />
                    </div>
                    <div>
                        <label class="block font-semibold text-sm mb-1">Mô tả</label>
                        <textarea v-model="createForm.description" rows="3" maxlength="2000" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none"></textarea>
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select v-model="createForm.category_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                                ...
                                <option v-for="c in createCategories" :key="c.id" :value="c.id">{{ c.name }}</option>
                            </select>
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <input v-model="createForm.notes" type="text" maxlength="2000" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" />
                        </div>
                    </div>
                </div>
                <div class="flex justify-end gap-3 px-6 py-4 border-t bg-white">
