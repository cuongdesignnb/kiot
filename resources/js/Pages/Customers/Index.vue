...
const submitDebtModal = async () => {
    ...
    if (type === "payment") {
        if (debtForm.mode === "manual") {
            ...
            try {
                const paymentAmount = Number(debtForm.amount) || manualTotal();
                if (paymentAmount < manualTotal()) {
                    alert("Số tiền thực nhận không được nhỏ hơn tổng phân bổ");
                    return;
                }
                const response = await axios.post(`/customers/${customerId}/debt-payment`, {
                    mode: "manual",
        ...
        >
            ...
            <div class="bg-white rounded-lg shadow-xl w-[540px] max-h-[90vh] overflow-y-auto relative z-10">
                ...
                <div class="px-6 py-4 space-y-4">
                    ...
                    <template v-if="debtModal.type === 'payment'">
                        ...
                        <div v-if="debtForm.mode === 'manual'">
                            ...
                            <MoneyInput v-model="debtForm.amount" placeholder="Bằng tổng phân bổ nếu để 0" input-class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500 mb-3" />
                    ...
                    <div v-if="debtPaymentResult" class="rounded-lg border border-green-200 bg-green-50 p-4">
                        <div class="mb-3 font-semibold text-green-800">Kết quả thu nợ</div>
                        <div class="grid grid-cols-2 gap-2 text-sm">
                            <span>Tiền thực nhận</span><strong class="text-right">{{ formatCurrency(debtPaymentResult.payment_amount) }}</strong>
                            <span>Đã phân bổ</span><strong class="text-right">{{ formatCurrency(debtPaymentResult.allocated_amount) }}</strong>
                            <span>Chưa phân bổ</span><strong class="text-right text-green-700">{{ formatCurrency(debtPaymentResult.unallocated_amount) }}</strong>
                            <span>Công nợ trước</span><strong class="text-right">{{ formatCurrency(debtPaymentResult.debt_before) }}</strong>
                            <span>Công nợ sau</span><strong class="text-right">{{ formatCurrency(debtPaymentResult.debt_after) }}</strong>
                        </div>
                    </div>
                ...
                >
                    ...
                    >
                        {{ debtPaymentResult ? 'Đóng' : 'Bỏ qua' }}
                    </button>
                    <button
                        v-if="!debtPaymentResult"
            ...
            <div class="bg-white rounded-lg shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
                ...
                <template v-else-if="invoiceDetail.data">
                    ...
                    <div class="px-6 py-3 grid grid-cols-2 gap-x-8 gap-y-2 text-sm border-b bg-gray-50">
                        ...
                        <div title="Thời gian giao dịch được dùng trong báo cáo và công nợ. Thời điểm ghi nhận là lúc hệ thống lưu hoàn tất chứng từ."><span class="text-gray-500">Thời điểm ghi nhận trên hệ thống:</span> <span class="font-medium">{{ invoiceDetail.data.recorded_at }}</span></div>
                    ...
                    <template v-else-if="debtVoucherDetailModal.data">
                        ...
                        <div v-if="debtVoucherDetailModal.type === 'invoice'" class="space-y-6">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-3 bg-gray-50 p-4 rounded border border-gray-100">
                                ...
                                <div title="Thời gian giao dịch được dùng trong báo cáo và công nợ. Thời điểm ghi nhận là lúc hệ thống lưu hoàn tất chứng từ."><span class="text-gray-500">Thời điểm ghi nhận trên hệ thống:</span> <span class="font-medium text-gray-800 ml-1">{{ debtVoucherDetailModal.data.recorded_at }}</span></div>
                        ...
                        <div v-if="debtVoucherDetailModal.type === 'purchase'" class="space-y-6">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-3 bg-gray-50 p-4 rounded border border-gray-100">
                                ...
                                <div><span class="text-gray-500">Thời gian giao dịch:</span> <span class="font-medium text-gray-800 ml-1">{{ debtVoucherDetailModal.data.business_time }}</span></div>
                                <div><span class="text-gray-500">Thời điểm ghi nhận trên hệ thống:</span> <span class="font-medium text-gray-800 ml-1">{{ debtVoucherDetailModal.data.recorded_at }}</span></div>
