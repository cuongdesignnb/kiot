...
const props = defineProps({
    filters: Object, periodLabel: String, chartData: Object,
    reportRows: Array, summary: Object, branchName: String,
    branches: Array, dateFromDisplay: String, dateToDisplay: String,
});

const concern = ref(props.filters.concern || "purchase");
...
const barChartData = computed(() => ({
    ...
    datasets: (props.chartData?.datasets || []).map((ds) => ({
        label: ds.label, data: ds.data, backgroundColor: "#3b82f6",
...
const barChartOptions = computed(() => ({
    ...
    scales: { x: { ticks: { callback: (v) => formatNumber(v) } }, y: { beginAtZero: true } },
}));
    ...
    <AppLayout>
        <div class="flex h-full min-h-[calc(100vh-56px)]">
            ...
            <aside class="w-[200px] bg-white border-r border-gray-200 p-4 flex-shrink-0 overflow-y-auto print:hidden">
                <h2 class="text-sm font-bold text-gray-800 mb-4">Báo cáo nhà cung cấp</h2>
                ...
                <template v-if="viewMode === 'chart'">
                    <div class="p-6">
                        <div class="text-center mb-4">
                            <h2 class="text-base font-semibold text-gray-700">{{ chartData?.title }}</h2>
                        </div>
                        <div class="bg-white rounded-lg border border-gray-200 p-5">
                            <div v-if="chartData?.labels?.length > 0"
                                :style="{ height: Math.max(300, chartData.labels.length * 45) + 'px' }">
                                <Bar :data="barChartData" :options="barChartOptions" />
                            </div>
                            <div v-else class="h-[300px] flex items-center justify-center text-gray-400">
                                <div class="text-center"><div class="text-4xl mb-2">🏭</div><div class="text-sm">Không có dữ liệu nhà cung cấp</div></div>
                            </div>
                        </div>
                    </div>
                </template>
                ...
                <template v-if="viewMode === 'report'">
                    ...
                    <div class="bg-[#404040] px-3 py-1.5 flex items-center justify-between print:hidden sticky top-0 z-10">
                        <div class="flex items-center gap-1">
                            <button @click="goFirst" class="pdf-btn" title="Trang đầu">⏮</button>
                            <button @click="goPrev" class="pdf-btn" title="Trang trước">◀</button>
                            ...
                            <button @click="goNext" class="pdf-btn" title="Trang sau">▶</button>
                            <button @click="goLast" class="pdf-btn" title="Trang cuối">⏭</button>
                        </div>
                        <div class="flex items-center gap-1">
                            ...
                            <span class="w-px h-5 bg-gray-500 mx-1"></span>
                            <button @click="zoomOut" class="pdf-btn" title="Thu nhỏ">🔍−</button>
                            <span class="text-white text-xs mx-1">{{ zoom }}%</span>
                            <button @click="zoomIn" class="pdf-btn" title="Phóng to">🔍+</button>
                        </div>
                    </div>
                    ...
                    <div class="p-6 flex justify-center bg-[#e8e8e8] min-h-[calc(100vh-120px)]">
                        <div class="bg-white shadow-lg border border-gray-300 w-full p-10 print:shadow-none print:border-none print:p-0 print:max-w-full"
                            ...
                            <h1 class="text-lg font-bold text-center mb-1">Báo cáo nhập hàng theo nhà cung cấp</h1>
                            ...
                            <table class="w-full border-collapse" style="font-size:0.85em">
                                <thead>
                                    <tr class="bg-[#c9960c] text-white">
                                        <th class="px-3 py-2 text-left font-semibold border border-[#b8870b]">Mã nhà cung cấp</th>
                                        <th class="px-3 py-2 text-left font-semibold border border-[#b8870b]">Tên nhà cung cấp</th>
                                        <th class="px-3 py-2 text-right font-semibold border border-[#b8870b]">Giá trị nhập</th>
                                        <th class="px-3 py-2 text-right font-semibold border border-[#b8870b]">Giá trị trả</th>
                                        <th class="px-3 py-2 text-right font-semibold border border-[#b8870b]">Giá trị (thuần)</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <!-- Summary row -->
                                    <tr class="bg-yellow-50 font-bold">
                                        <td colspan="2" class="px-3 py-2 border border-gray-200">SL NCC: {{ summary?.count || 0 }}</td>
                                        <td class="px-3 py-2 text-right border border-gray-200">{{ formatCurrency(summary?.totalImport) }}</td>
                                    ...
                                    <tr v-for="row in paginatedRows" :key="row.id" class="hover:bg-gray-50">
                                        ...
                                        <td class="px-3 py-1.5 border border-gray-200 text-right">{{ formatCurrency(row.import) }}</td>
