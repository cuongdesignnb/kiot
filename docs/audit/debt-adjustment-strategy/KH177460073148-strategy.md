...
- Timeline cashflow entry: `no`.
- Timeline final balance: `15000000`.
- CustomerDebt rows: invoice `0`, cashflow `0`.
- Reconcile mismatch: `yes`.

## Strategy comparison

| Strategy | Write DB | Tables affected | Risk | Pros | Cons | Recommended |
|---|---|---|---|---|---|---|
| DISPLAY_ONLY_TIMELINE_FIX | false |  | LOW-MEDIUM | Khong sua du lieu cu; Tranh double-count; Phu hop vi stored debt hien tai da la 0 | Khong tao ledger that; Chi giai thich hien thi/timeline; Can test de khong anh huong phieu thu thuong | yes |
...
- Reason: Stored debt is 0, invoice +15M is shown, matching DebtAdjustment cashflow is missing from timeline..
