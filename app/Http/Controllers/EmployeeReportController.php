...
class EmployeeReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($salesChannel) {
            $invoiceQ->where('sales_channel', $salesChannel);
        }
        ...
        $returnQ = OrderReturn::whereBetween('created_at', [$startDate, $endDate])
            ->where('status', '!=', 'Đã hủy');
        if ($branchId) {
            $returnQ->where('branch_id', $branchId);
        }
    ...
    private function buildSalesDailyChildren($invoiceQ, $returnQ, array $filters): array
    {
        ...
        if ($returnRows->isNotEmpty()) {
            $returnSellerMap = $this->sellers->returnSellerMap(clone $returnQ);

            foreach ($returnRows as $ret) {
                $sellerKey = $returnSellerMap[$ret->id] ?? 'unknown';
