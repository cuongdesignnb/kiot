...
class TaskController extends Controller
{
    ...
    /**
     * Tạo sản phẩm nhanh (từ modal linh kiện).
     */
    public function quickCreateProduct(Request $request)
    {
        ...
        // Auto-generate SKU
        $prefix = 'SP';
        $last = Product::orderByDesc('id')->value('sku');
        $num = 1;
        if ($last && preg_match('/(\d+)$/', $last, $m)) {
            $num = (int) $m[1] + 1;
        }
        $sku = $prefix . str_pad($num, 6, '0', STR_PAD_LEFT);
    ...
    /**
     * Xuất nhiều linh kiện cùng lúc.
     */
    public function batchAddParts(Request $request, Task $task)
    {
        if (!$task->is_repair) {
            return response()->json(['message' => 'Chỉ phiếu sửa chữa mới có linh kiện.'], 422);
        }
    ...
    /**
     * Bóc nhiều linh kiện cùng lúc.
     */
    public function batchDisassembleParts(Request $request, Task $task)
    {
        if (!$task->is_repair) {
            return response()->json(['message' => 'Chỉ phiếu sửa chữa mới có thể bóc linh kiện.'], 422);
        }
