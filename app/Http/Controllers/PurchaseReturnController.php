...
class PurchaseReturnController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $return = app(\App\Services\Debt\PartnerDebtMutationCoordinator::class)->execute(
                (int) $purchase->supplier_id,
                'purchase_return_create',
                $payloadHash,
                function (Customer $lockedSupplier) use ($purchase, $request): PurchaseReturn {
                    ...
                    if ($purchase->supplier) {
                        ...
                        $lockedSupplier->supplier_debt_amount -= $debtReduction;
                        $lockedSupplier->total_bought -= $totalAmount;
                        $lockedSupplier->save();
                    }
    ...
    public function quickStore(Request $request)
    {
        ...
        try {
            ...
            $return = app(\App\Services\Debt\PartnerDebtMutationCoordinator::class)->execute(
                (int) $request->supplier_id,
                'quick_purchase_return_create',
                $payloadHash,
                function (Customer $lockedSupplier) use ($request): PurchaseReturn {
                    app(\App\Services\PartnerTransactionGuard::class)->assertCanTransact(
                        (int) $request->supplier_id,
                        'supplier_id'
                    );
    ...
    public function destroy(PurchaseReturn $purchaseReturn)
    {
        ...
        try {
            $payloadHash = hash('sha256', json_encode([
                'purchase_return_id' => (int) $purchaseReturn->id,
                'supplier_id' => (int) $purchaseReturn->supplier_id,
            ], JSON_UNESCAPED_UNICODE));
            $purchaseReturn = app(\App\Services\Debt\PartnerDebtMutationCoordinator::class)->execute(
                (int) $purchaseReturn->supplier_id,
                'purchase_return_cancel',
                $payloadHash,
                function (Customer $lockedSupplier) use ($purchaseReturn): PurchaseReturn {
                    $purchaseReturn = PurchaseReturn::with(['items', 'supplier'])->lockForUpdate()->findOrFail($purchaseReturn->id);
                    if (! (bool) $lockedSupplier->is_supplier) {
                        throw ValidationException::withMessages([
                            'supplier_id' => 'Đối tác chưa có vai trò nhà cung cấp đã lưu.',
                        ]);
                    ...
                    if ($purchaseReturn->supplier) {
                        ...
                        $lockedSupplier->supplier_debt_amount += $debtReduction;
                        $lockedSupplier->total_bought += $purchaseReturn->total_amount;
                        $lockedSupplier->save();
                    }
