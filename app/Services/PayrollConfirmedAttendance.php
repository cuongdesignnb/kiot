...
{
    public function summarize(Collection $records, array $holidays, float $restRate, float $holidayRate): array
    {
        ...
        foreach ($records->groupBy(fn ($r) => Carbon::parse($r->work_date)->toDateString()) as $date => $rows) {
            ...
            if ($rows->contains(fn ($r) => $r->needs_review)) {
                $issues[] = "Chấm công ngày {$date} đang cần kiểm tra.";
            }
            if ($rows->contains(fn ($r) => $r->work_units === null || (float) $r->work_units < 0)
