...
class DebtPartnerInspectionService
{
    ...
    private function partnerPayload(Customer $partner): array
    {
        return [
            'id' => $partner->id,
            'code' => $partner->code,
            'name' => $this->normalizeText($partner->name),
            'phone' => $partner->phone,
    ...
    private function normalizeText(mixed $value): ?string
    {
        ...
        $fixed = $this->decodeMojibakeOnce($text);

        return $fixed !== null && $this->textScore($fixed) > $this->textScore($text)
            ? $fixed
            : $text;
    }
    ...
    {
        return preg_match('/(?:[ÃÂÆÄâ€\x{0080}-\x{009F}]|á[º»])/u', $text) === 1;
    }

    private function decodeMojibakeOnce(string $text): ?string
    {
        $map = [
    ...
    private function textScore(string $text): int
    {
        $score = 0;
        $score -= preg_match_all('/[ÃÂÆÄâ€\x{0080}-\x{009F}]/u', $text) * 3;
        $score += preg_match_all('/[ăâđêôơưĂÂĐÊÔƠƯáàảãạấầẩẫậắằẳẵặéèẻẽẹếềểễệíìỉĩịóòỏõọốồổỗộớờởỡợúùủũụứừửữựýỳỷỹỵÁÀẢÃẠẤẦẨẪẬẮẰẲẴẶÉÈẺẼẸẾỀỂỄỆÍÌỈĨỊÓÒỎÕỌỐỒỔỖỘỚỜỞỠỢÚÙỦŨỤỨỪỬỮỰÝỲỶỸỴ]/u', $text);
